...
inline int BuildTree(pq_point& z, pq_point& z_propose, 
  ...
  if(depth == 0){
    ...
    float joint = postparams.logfullcondit(z.q) - 0.5 * arma::conv_to<double>::from(z.p.t() * //postparams.M * 
                                                                                    z.p);//sum(z.p % z.p); 
...
inline arma::mat sample_one_mala_cpp(arma::mat current_q, 
                                     T& postparams,
                                     AdaptE& adaptparams, bool rm=true, bool adapt=true){
  ...
  arma::vec proposal_mean = veccurq + eps2 * 0.5 * Minv * xgrad;// / max(eps2, arma::norm(xgrad));
  ...
  arma::vec reverse_mean = q + eps2 * 0.5 * Minv * revgrad;// / max(eps2, arma::norm(revgrad));;
