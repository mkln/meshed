...
arma::vec Meshed::sample_BetaLambda_row(int j, const arma::mat& rnorm_precalc){
  ...
  arma::mat Lprior_inv = lambda_prec * arma::eye( XW.n_cols, XW.n_cols );
  ...
  arma::mat BLrow_Sig = Sigma_chol_L.t() * Sigma_chol_L;
  arma::mat BLrow_mu = Lprior_inv * Lprior_mean + 
    BLrow_Sig * Simean_L;

  arma::vec sampled = BLrow_mu + Sigma_chol_L.t() * 
        arma::trans(rnorm_precalc.submat(j, 0, j, XW.n_cols-1));

  return sampled;
}

arma::vec Meshed::update_BetaLambda_row(int j, const arma::mat& rnorm_precalc){
  // non-Gaussian update
  ...
  arma::mat BL_Vi = lambda_prec * arma::eye( XW.n_cols, XW.n_cols );
  BL_Vi.submat(0, 0, p-1, p-1) = Vi; // prior precision for beta
  arma::vec BL_Vim = arma::zeros(XW.n_cols);
  
  BL_node.at(j).update_mv(offsets_obs, 1.0/tausq_inv(j), BL_Vim, BL_Vi);
  BL_node.at(j).X = XW;
  ...
  // nongaussian
  BL_hmc_adapt.at(j).step();
  if((BL_hmc_started(j) == 0) && (BL_hmc_adapt.at(j).i == 10)){
    // wait a few iterations before starting adaptation
    //Rcpp::Rcout << "reasonable stepsize " << endl;
    
    double lambda_eps = find_reasonable_stepsize(curLrow, BL_node.at(j), rnorm_row);
    
    int n_params = curLrow.n_elem;
    AdaptE new_adapting_scheme;
    new_adapting_scheme.init(lambda_eps, n_params, which_hmc, 1e4);
    BL_hmc_adapt.at(j) = new_adapting_scheme;
    BL_hmc_started(j) = 1;
    //Rcpp::Rcout << "done initiating adapting scheme" << endl;
  }
  if(which_hmc == 0){
    // some form of manifold mala
    sampled = simpa_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j), 
                        rnorm_row, lambda_runif(j), lambda_runif2(j), 
                        debug);
  }
  if(which_hmc == 1){
    // mala
    sampled = mala_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j), 
                       rnorm_row, lambda_runif(j), debug);
  }
  if(which_hmc == 2){
    // nuts
    sampled = nuts_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j)); 
  }
  
  if(which_hmc == 3){
    // some form of manifold mala
    sampled = smmala_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j), 
                         rnorm_row, lambda_runif(j), debug);
  }
  if(which_hmc == 6){
    sampled = hmc_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j), 
                      rnorm_row, lambda_runif(j), 0.1, debug);
  }
  if(which_hmc == 7){
    // some form of manifold mala
    sampled = yamala_cpp(curLrow, BL_node.at(j), BL_hmc_adapt.at(j), 
                         rnorm_row, lambda_runif(j), lambda_runif2(j), 
                         debug);
  }
...
void Meshed::deal_with_BetaLambdaTau(bool sample_beta, bool sample_lambda, bool sample_tau){
  if(verbose & debug){
...
#pragma omp parallel for 
  ...
  for(unsigned int j=0; j<q; j++){
    ...
    if(familyid(j) == 0){
      // Gibbs update
      sampled = sample_BetaLambda_row(j, rnorm_precalc);
    } else {
      // non-Gaussian update 
      sampled = update_BetaLambda_row(j, rnorm_precalc);
    }
    ...
    ///
    /// ** tausq update for gaussian/beta/negbinom outcomes **
    ///
