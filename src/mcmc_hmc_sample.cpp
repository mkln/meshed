...
template <class T>
void leapfrog(pq_point &z, float epsilon, T& postparams,  int k){
  arma::mat qmat = arma::mat(z.q.memptr(), z.q.n_elem/k, k);
  ...
  z.q += epsilon * z.p;
  qmat = arma::mat(z.q.memptr(), z.q.n_elem/k, k);
  z.p += epsilon * 0.5  * postparams.gradient_logfullcondit(qmat);
}
...
arma::mat hmc_cpp(arma::mat current_q, 
  ...
  int k = current_q.n_cols;
  arma::mat newq = arma::mat(z.q.memptr(), z.q.n_elem/k, k);
...
double find_reasonable_stepsize(const arma::mat& current_q, T& postparams, const arma::mat& rnorm_mat){
  ...
  while( condition & (it < 50) ){
    ...
    leapfrog(z, epsilon, postparams, current_q.n_cols);
    newq = arma::mat(z.q.memptr(), z.q.n_elem/k, k);
  ...
  while(util.criterion && (depth_ < MAXDEPTH)){
    ...
    if(util.criterion){ 
      ...
      if(R::runif(0, 1) < subtree_prob){ 
        arma::mat newq = arma::mat(z_propose.q.memptr(), z_propose.q.n_elem/ksize, ksize);
